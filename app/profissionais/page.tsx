...
import { fetchProfessionals } from "@/lib/data-service"
...
export default function ProfissionaisPage() {
  ...
  const filteredProfessionals = professionals.filter((prof) => {
    ...
    const matchesState = selectedState === "Todos" || prof.state === selectedState

    const matchesCity = selectedCity === "" || prof.city?.toLowerCase().includes(selectedCity.toLowerCase())

    const matchesRating = (prof.averageRating || 0) >= minRating[0]

    return matchesSearch && matchesSpecialty && matchesState && matchesCity && matchesRating
  })
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">
              Encontre fotógrafos, videomakers e profissionais de audiovisual qualificados
            </p>
          </div>
          ...
          {loading ? (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                <p className="text-muted-foreground">Nenhum profissional encontrado com os filtros aplicados.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProfessionals.map((prof) => (
                <Card key={prof.uid} className="hover:border-primary transition-colors">
                  <CardContent className="pt-6">
                    <Link href={`/profissionais/${prof.uid}`}>
                      <div className="text-center space-y-4">
                        ...
                        <div className="relative mx-auto h-24 w-24 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden">
                          {prof.photoURL ? (
                            <img
                              src={prof.photoURL || "/placeholder.svg"}
                        ...
                        {/* Info */}
                        <div>
                          <h3 className="font-semibold text-lg">{prof.artisticName || prof.displayName}</h3>
                          {prof.specialty && <p className="text-sm text-muted-foreground">{prof.specialty}</p>}
                        </div>
                        ...
                        {prof.totalReviews && prof.totalReviews > 0 ? (
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <span className="text-sm text-muted-foreground">({prof.totalReviews})</span>
                          </div>
                        ) : (
